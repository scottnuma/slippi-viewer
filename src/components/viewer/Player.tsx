...
import { RenderData } from "../../state/types";
import { ReplayData } from "../../common/types";
export function Players() {
  const renderDatas = useReplayStore((state) => state.renderDatas);
      ...
      <For each={renderDatas}>
        {(renderData) => (
          <>
            ...
            />
            <Shield renderData={renderData} replayData={replayData} />
...
function Shield(props: { renderData: RenderData, replayData?: ReplayData }) {
  // [0,60]
  ...
  const shieldSizeMultiplier = () => ((shieldHealth() * triggerStrengthMultiplier()) / 60) * 0.85 + 0.15
  return (
    <>
      <Show
