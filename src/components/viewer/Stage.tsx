...
export function Stage() {
  ...
  const stageName = useCallback(() => stageNameByExternalId[replayData!.settings.stageId], [replayData]);
  return (
    <Switch>
...
function Battlefield() {
  ...
  return (
    <>
      ...
      <For each={platforms}>
        {(points) => (
          <polyline points={points.join(" ")} className="stroke-slate-800" />
        )}
      </For>
      <rect
        x={blastzones[0][0]}
...
function YoshisStory(props: { frame: number }) {
  const mainStage = [
    "-54, -91",
    "-54, -47",
    "-53, -46",
  ...
  const randall = () => {
    ...
    const frameInLap = (props.frame - 123 + 1200) % 1200;
