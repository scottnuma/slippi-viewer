import {For, Match, Switch} from '../conversion/helpers';
import { itemNamesById } from "../../common/ids";
import { ItemUpdate, PlayerUpdate, ReplayData } from "../../common/types";
  ...
  return (
    <Switch>
      ...
      <Match when={itemName() === "Turnip"}>
        <Turnip item={props.item} replayData={replayData!} />
      </Match>
      <Match when={itemName() === "Yoshi's egg(thrown)"}>
        <YoshiEgg item={props.item} replayData={replayData!} />
      </Match>
...
function YoshiEgg(props: { item: ItemUpdate, replayData: ReplayData }) {
  // states: 0 = held, 1 = thrown, 2 = exploded
  const ownerState = () => getOwner(props.replayData, props.item).state;
  return (
    <>
      <circle
        cx={
...
function Turnip(props: { item: ItemUpdate, replayData: ReplayData }) {
...
function getOwner(replayData: ReplayData, item: ItemUpdate): PlayerUpdate {
  return replayData.frames[item.frameNumber].players[item.owner];
}
